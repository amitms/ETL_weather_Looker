...
def weather_pipeline():
  @task
  def retrieve_weather(city, start_day, end_day):
    ...
    # Make the request to the visual Crossing Weather API
        with urllib.request.urlopen(request_url) as response:
          if response.status == 200:
            ...
            print("weather_data.json will be passed to transformed ..")
            logger.info("weather_data.json will be passed to transformed ..")
  ...
  data = retrieve_weather("FortWorth,USA","2026-03-18","2026-03-18") 
  transformed = transform_data(data) 
  load_data_to_db2(transformed, table_name="weather_data")


# step3: defining tasks with @task decorators in the python methods
# Instantiate DAG
weather_etl = weather_pipeline()
